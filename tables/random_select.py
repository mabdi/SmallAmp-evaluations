          ...
          with open(file) as f:
              ...
              for row in tsv:
                 if float(row[5]) >= 50:
                   ...
                   l.append(row)
              l_selected = random.sample(l, min(len(l), 2 + 2 - min(len(h), 2)))
              h_selected = random.sample(h, min(len(h), 2 + 2 - min(len(l), 2)))
              project = file[:-(len('-details.tsv'))]
              data.extend(prepare_list(l_selected, project, 'L'))
              data.extend(prepare_list(h_selected, project, 'H'))
